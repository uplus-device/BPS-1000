...
static OMX_STRING ParseEnumValue(ConfigEnum *pConfigEnum, OMX_S32 pEnumVal)
{
    ...
    VTEST_MSG_ERROR("error could not find enum value");
	return NULL;
}
    ...
    while (pExtra && ((OMX_U8*)pExtra < (pBaseAdd + nBuffer_size)) &&
        ...
        switch ((unsigned long)pExtra->eType)
        {
            ...
            case OMX_ExtraDataMP2UserData: {
                ...
                if (m_fUserData && m_fUserDataRef)
                {
                    OMX_STRING userdata_type = (OMX_STRING)ParseEnumValue(g_sessionUserDataTypeMap, userdata->type);
                    if (userdata_type)
                        fprintf(m_fUserDataRef, "%u %u %s ", m_nFrameDoneCount, userdata_size, userdata_type);
