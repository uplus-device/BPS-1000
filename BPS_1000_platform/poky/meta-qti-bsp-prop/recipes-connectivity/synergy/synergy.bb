...
if [ -f "${B}/synergy-bt/platform/mdm/makefile" ]; then
    make -C ${B} -f ${B}/synergy-bt/platform/mdm/makefile all IMGRFS=${STAGING_DIR_HOST} CROSS_COMPILE=${TARGET_PREFIX} V=1 CC="${CC}"
fi
}
...
do_compile_sdk () {
    ...
    make -C ${B} -f ${B}/synergy-bt-sdk/platform/mdm/makefile all IMGRFS=${STAGING_DIR_HOST} CROSS_COMPILE=${TARGET_PREFIX} V=1 CC="${CC}"
}
